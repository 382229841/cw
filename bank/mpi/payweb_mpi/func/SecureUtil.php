...
include_once $_SERVER ['DOCUMENT_ROOT'] . '/ebank/mpi/payweb_mpi/func/PublicEncrypte.php';
...
function sign(&$params) {
	...
	$log->LogInfo ( "签名串 >" . $params_str );
	
	// 签名证书路径
	$cert_path = SDK_SIGN_CERT_PATH;
	$private_key = getPrivateKey ( $cert_path );
	// 签名
	$sign_falg = openssl_sign ( $params_str, $signature, $private_key, OPENSSL_ALGO_SHA1 );
	if ($sign_falg) {
...
function verify($params) {
	...
	$params_str = coverParamsToStringVertify( $params,$signature );
	$log->LogInfo ( '验签，验证签名字符串>' . $params_str);

	$cert_path = SDK_VERIFY_CERT_PATH;
	$publicKey = getPublicKey($cert_path);
	$res = openssl_get_publickey($publicKey);
	$result = openssl_verify($params_str, base64_decode($signature), $res);
