  ...
  <head>
	...
	<script>
		...
		var weixinSdkConfig=function(ticket){
		 ...
		 var url="http://d.xiaoniubang.com/api/access_token.php";
		 $.ajax({
			url: url,
			dataType: "json",
			cache:true,
		...
		$("#return").bind("click",function(){
			window.location='http://d.xiaoniubang.com/#/recharge';
		});
		
		$("#pay").bind("click",function(){
			if(getQueryStringByName('rechargeType')==1){//支付宝付款
			    window.location.href="http://d.xiaoniubang.com/pay/createTransaction.php?uid="+r.uid
					+"&no="+r.no
					+"&amount="+r.amount
					+"&rechargeType=4"
			...
			if(getQueryStringByName('rechargeType')==2)//微信支付充值
			{
				...
				*/
				var url="http://d.xiaoniubang.com/api/pay/wechat/demo/getPayparam.php?openid="
						...
						if (data) {
							...
							wx.chooseWXPay({
								...
								success: function (res) {
									...
									window.location.href="http://d.xiaoniubang.com/pay/createTransaction-wx.php?uid="+r.uid
									+"&no="+r.no
									+"&amount="+r.amount
									+"&rechargeType=4"
									+"&productId="+r.productId
