...
<html>
  <head>
	...
	<script>
		...
		$("#return").bind("click",function(){
			window.location='http://m.xiaoniubang.com/ebank/#/recharge';
		});
		...
		$("#pay").bind("click",function(){
			...
			if(getQueryStringByName('rechargeType')==4){//苏州银行手机银行
			    window.location.href="http://m.xiaoniubang.com/ebank/mpi/proccess/TransProcess.php?user_id="+r.uid
					+"&orderNumber="+r.no
			...
			if(getQueryStringByName('rechargeType')==2)//微信支付充值
			{
				...
				*/
				var url="http://m.xiaoniubang.com/api/pay/wechat/demo/getPayparam.php?openid="
					+r.openid+"&amount="+r.amount+"&productId="+r.productId+"&uid="+r.uid;//+r.amount;
					//alert(url);
  ...
  <body style="padding:1em .5em;text-align:center;overflow-y:auto;">
    ...
    <div class="address-info">
            ...
            <div>
				<button id="pay">确定</button>
				<button id="pay2" style="display:none;">正在进行微信安全支付。。。</button>
			</div>
