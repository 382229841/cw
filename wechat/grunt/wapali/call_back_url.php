...
LogModel::Write('alipay',$str,'call_back_url');
?>
<!DOCTYPE HTML>
<html>
    <head>
...
if($verify_result) {//验证成功
    ...
    //支付价格
    $pay_much = $_GET['total_fee'];

    $sell_buy = $_GET['buyer_email'];
    if($_GET['result'] == 'success') {
        $code[] = $out_trade_no;
       // $code[] = $pay_much;
        $code[] = $trade_no;
        $pay_sn = encrypt(implode('_#_',$code),$key);
        $pay_sn = setUrlCode($pay_sn);
        Header("Location: ".$alipay_config['paySuccess']."?trade_no=$trade_no&order_id=$out_trade_no";);
    }
    else {
